...
const CONVERSATION_COPY = {
  'zh-CN': {
    ...
    permissions: {
      menuTitle: '权限模式',
      mode: {
        explore: { label: '只读', hint: 'Maka 只读取和搜索，不修改文件、不运行有副作用的命令；联网搜索与抓取不受限。' },
        ask: { label: '手动', hint: 'Maka 修改工作目录以外的文件前，会先询问你；联网不受限。' },
    ...
    sandboxBoundary: {
      ...
      consequence: {
        read: 'Maka 将能在本会话中读取这里的文件。',
        write: 'Maka 将能在本会话中读取和修改这里的文件。',
        network: 'Maka 将能在本会话中访问网络。',
      },
    ...
    clientCapability: {
      ...
      consequence: {
        browser: 'Maka 将能在本会话中读取并操作这个网站。',
        computerUse: 'Maka 将能在本会话中查看屏幕，并操作这台 Mac 上的应用。',
        desktopMcp: 'Maka 将能在本会话中调用这个工具。',
      },
      label: { site: '网站', server: '服务器', tool: '工具' },
      sessionNotice: '允许后，本会话中相同范围的后续操作将不再询问。',
    },
  ...
  'zh-TW': {
    ...
    permissions: {
      menuTitle: '權限模式',
      mode: {
        explore: { label: '只讀', hint: 'Maka 只讀取和搜尋，不修改檔案、不執行有副作用的命令；連網搜尋與擷取不受限。' },
        ask: { label: '手動', hint: 'Maka 修改工作目錄以外的檔案前，會先詢問你；連網不受限。' },
    ...
    sandboxBoundary: {
      ...
      consequence: {
        read: 'Maka 將能在本工作階段中讀取這裡的檔案。',
        write: 'Maka 將能在本工作階段中讀取和修改這裡的檔案。',
        network: 'Maka 將能在本工作階段中存取網路。',
      },
    ...
    clientCapability: {
      ...
      consequence: {
        browser: 'Maka 將能在本工作階段中讀取並操作這個網站。',
        computerUse: 'Maka 將能在本工作階段中查看螢幕，並操作這台 Mac 上的應用程式。',
        desktopMcp: 'Maka 將能在本工作階段中呼叫這個工具。',
      },
      label: { site: '網站', server: '伺服器', tool: '工具' },
      sessionNotice: '允許後，本工作階段中相同範圍的後續操作將不再詢問。',
    },
  ...
  en: {
    ...
    permissions: {
      ...
      mode: {
        explore: { label: 'Read only', hint: 'Maka only reads and searches: no file edits, no commands with side effects. Web search and fetch are not restricted.' },
        ask: { label: 'Manual', hint: 'Maka asks before changing files outside the workspace; the network is not restricted.' },
