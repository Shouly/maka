...
describe('ShellRunProcessManager', () => {
  ...
  test('joins finalization when a real PTY exits before a queued control cut', async () => {
    ...
    // Hold one parser write so the resize queues behind a real cut while the
    // process exits. An output flood used to hold the cut, but queued output
    // now merges into one write and no longer delays control.
