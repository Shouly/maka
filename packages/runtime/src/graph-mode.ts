...
export function renderGraphModePrompt(): string {
  return [
    ...
    'Use UpdateAgentGraph to schedule bounded work onto dynamically provisioned child-Session operators when a graph materially improves the task.',
    'Use ViewAgentGraph with {"mode":"latest"} to observe committed records, readiness, waits, failures, and durable schedule state; use mode=page only with a returned nextCursor.',
    'After scheduling a wave, keep using any tools needed for immediate supervision. When only child execution remains, call YieldAgentGraph instead of polling, sleeping, or writing a waiting message. Yield ends only the current supervisor turn; the host will wake you at the next durable graph checkpoint and the graph remains open.',
    ...
    'Before advancing dependencies or finishing, read the committed child result with AgentOutput like {"locator":"child_session_run","child_session_id":"<childSessionId>","run_id":"<currentRunId>","view":"result","max_bytes":32768}. Use result.resultRecordId when selecting a final committed record. Read runtime_events or view=all only for a narrow diagnostic question. Then select committed result ids with UpdateAgentGraph.finish and synthesize those results for the user.',
    ...
    'For a result selected by a finished earlier graph epoch, use selected_result_inputs with the exact source_graph_id and result_id returned by ViewAgentGraph. Keep input_ids for records in the current graph; historical inputs carry data lineage only and never reuse old operators or control state.',
