...
export function renderSwarmModePrompt(): string {
  return [
    ...
    'Call ListAgents first and select an available user-approved subagent_id. Schedule all independent items together with UpdateAgentGraph using target_kind=new_preset and that subagent_id.',
    'After scheduling, call YieldAgentGraph. Do not poll, sleep, watch child logs, or wait synchronously; the host will wake you only when work needs attention or the whole swarm is settled.',
    'On wake, call SwarmStatus for compact statuses only. Read AgentOutput view=result only for completed final results or narrow failed-item diagnosis.',
    'Replace failed work with UpdateAgentGraph using replaces=<failed work id> and replacement_mode=replace so the failed item no longer keeps the swarm in needs_attention. When all useful work is settled, finish the graph, deduplicate, verify, and semantically synthesize the results.',
