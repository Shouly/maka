...
export class CurrentTaskStore {
  private snapshot: TaskQuerySnapshot = { status: 'idle', items: [] };
  ...
  constructor(
    private readonly reader: { read(sessionId: string): Promise<SessionTaskDocument> },
    private readonly onChange: () => void = () => undefined,
  ) {}

  getState(): TaskQuerySnapshot {
  ...
  private async load(sessionId: string, generation: number): Promise<boolean> {
    ...
    try {
      ...
      const items = projectSessionTasksForDisplay(result.items);
...
/** Render the one-line current task indicator. Empty and initial states stay hidden. */
export function renderTaskIndicator(
  snapshot: TaskQuerySnapshot,
  input: TaskIndicatorInput,
): string | undefined {
  const copy = TUI_COPY_RESOURCES.tasks[input.locale];
...
export interface TaskOverlayInput {
  locale: UiLocale;
  getState: () => TaskQuerySnapshot;
  viewportRows: () => number;
  onClose: () => void;
...
/** Read-only, scrollable view of the current task list. */
export class TaskOverlay implements Component {
  private top = 0;
  private bodyRows = 1;
  private width = 80;

  constructor(private readonly input: TaskOverlayInput) {}
  ...
  render(width: number): string[] {
    ...
    const copy = TUI_COPY_RESOURCES.tasks[this.input.locale];
  ...
  private rows(snapshot: TaskQuerySnapshot, width: number): string[] {
    const copy = TUI_COPY_RESOURCES.tasks[this.input.locale];
    ...
    return snapshot.items.flatMap((item) => {
      ...
      const blocked =
        item.blockedBy.length > 0
          ...
          : '';
      return wrapTaskItem(`${symbol} `, `${label}${blocked}`, width);
    });
  }
...
function wrapTaskItem(prefix: string, content: string, width: number): string[] {
