...
test('a fresh Host without an environment key starts with no Connection', async () => {
...
test('an interrupted environment import resumes and selects its default', async () => {
  await withFixture(async ({ root, stores }) => {
    ...
    const anthropic = catalog.connections[0]!;
    ...
    // Finished: a later start changes nothing.
    await ensureBootstrapRuntimePolicy({ workspaceRoot: root, stores, environment: {} });
...
test('a failed environment credential leaves no unusable default and is retried', async () => {
  await withFixture(async ({ root, stores }) => {
...
test('a stored OpenCode Free default is released, the Connection kept, nothing imported', async () => {
  await withFixture(async ({ root, stores }) => {
    ...
    // An import a previous build left unfinished must not bring in a paid key
    // in place of the retired default.
    await writeFile(
    ...
    assert.equal(catalog.defaultTarget, null);
    assert.deepEqual(
      catalog.connections.map(({ slug, providerType }) => ({ slug, providerType })),
      [{ slug: 'opencode-free', providerType: 'opencode-free' }],
    );
    assert.deepEqual((await stores.credentialVault.getSnapshot()).entries, []);
