...
export interface ProviderDefaults {
   ...
   * user can finish setting up without leaving Maka: no endpoint to know —
   * a plan or an account sign-in. The account sign-ins reach the shortlist
   * through their OAuth cards, so the keyed providers here are plans.
   */
...
const providerRegistry = {
  ...
  'opencode-free': {
    ...
    modelDiscovery: {
      kind: 'fallback',
      reason: 'Retired: Maka no longer offers the anonymous OpenCode free tier.',
    },
    category: 'overseas',
  },
  togetherai: {
 ...
 * ships. This is the only reader of `fallbackModels` — a provider's offline
 * offer has exactly one authority.
 */
