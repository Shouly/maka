...
`TodoRead` returns the whole ordered list; `TodoWrite` atomically replaces the
whole ordered list.
...
```text
model TodoRead / TodoWrite            Desktop read-only panel
              \                         /
...
`TodoWrite` is an internal Host tool port rather than a public Client mutation
...
The first Host read of an uninitialized Session, through either `TodoRead` or
`session.todo.query`, persists an empty document and returns it. The first
explicit `TodoWrite` writes the requested complete list directly.
...
CLI/TUI renders the settled semantic `TodoRead` or `TodoWrite` tool result.
It does not present `TodoWrite` arguments as committed state, including when a
...
prompt. The model reads it on demand with `TodoRead`.
