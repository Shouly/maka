...
- 思考的命名（2026-09-24 读 `shared-12` 的 `_h`/`gh`/`yh` 和 `shared-11` 的首行工具，并在 claude.ai 实测）：参考用这段思考的
  摘要，没有摘要时用第一行有字的内容（去掉 markdown，截到 200 字），多段思考用"Thought for Ns"，最后才是"Thought process"。
  Maka 没有摘要，也没有每段思考的起止时间，所以只有一段思考、中间没有播报的 run，以及卡片里的每个思考步骤，都用第一行命名。
  其余情况仍是"思考过程"。偏离：参考展开时会去掉已经当标题的第一行，只有一行的思考干脆不能展开，因为它的标题是生成的短摘要。
  Maka 的第一行是模型原话，常常是一整段、被行宽截断，所以展开后保留全文。
