...
12. **Desktop shell chrome is Maka's, not relx's.** relx is a web app: its sidebar starts with a logo header and its page header is a second bar below it. A desktop window has a system titlebar row, and the app chrome must live in that row. The rules (owner decision 2026-09-07):
    - **Row 1 is the window titlebar** (`--h-titlebar: 36px`, matches main's `TITLEBAR_OVERLAY_HEIGHT`). It is one absolute, transparent, full-width strip (`.maka-window-titlebar`) and the app's only `-webkit-app-region: drag` surface; every control inside it is `no-drag`, and `dialog:modal` is `no-drag`. Gutters come from what the OS reports: left `max(24px, env(titlebar-area-x, 0px))` (macOS traffic lights sit at x 17 / y 14 with `titleBarStyle: hiddenInset`), right `max(24px, 100vw - env(titlebar-area-x) - env(titlebar-area-width))` (Windows caption buttons with `titleBarOverlay`); Linux gets the 24px floors.
    - The strip is a three-column grid aligned to the shell below it: **column 1** (width = `--maka-sidenav-width` minus gutter and gap, `auto` when collapsed) holds the **sidebar collapse/expand toggle and the search button, immediately right of the traffic lights** (`data-maka-contract="shell-topbar-rail"`); **column 2** (content column, flush with the seam) holds the session identity — inline-renameable title, project name, branch/revision badges (`data-maka-contract="titlebar-identity"`); **column 3** holds right-side actions (model switcher, later the workbar toggle).
    - The sidebar column and the content column both extend up under the strip (they paint the titlebar's background: `--sidebar-background` on the left, `--surface-1` on the right) and start their content at `padding-top: var(--h-titlebar)`. **No logo header in the sidebar**: the sidebar's first row is "New task", then the filter, then the list; the RELX wordmark belongs to the welcome/empty state and About.
