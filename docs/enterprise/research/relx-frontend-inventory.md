...
- Also: `--fill-brand #c6613f` / `--fill-brand-hover #d97757` (brand orange; this IS the fill of every solid send/confirm key — composer send, ask-user submit, knowledge composer send — and the accent role quad is not: accent appears as a tint behind blue text, never as a solid action fill), `--fill-primary` (= text-primary light, `#ffffff` dark) / `--fill-primary-hover` (`#2c2c2a`/`#e1e0d9`) / `--on-primary`, `--fill-secondary` `rgb(255 255 255 / 10%)` / `--fill-secondary-hover`, `--fill-field`, `--on-overlay #ffffff`.
