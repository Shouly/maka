...
`packages/runtime/src/tool-output.ts` states in a header comment that it is adapted from opencode. The statement is accurate and the adaptation is substantial: `truncateToolOutput` follows `truncate.output()` line for line, sharing the `2000`-line and `50 KiB` budgets, the `direction?: 'head' | 'tail'` option, the local names `out`, `bytes`, `hitBytes`, `size`, `preview`, `removed`, and `unit`, and the asymmetric newline accounting that uses `(i > 0 ? 1 : 0)` when keeping the head and `(out.length > 0 ? 1 : 0)` when keeping the tail. Maka added byte-safe single-line slicing, trailing-newline handling, and the recovery hint, and dropped the upstream spill-file machinery.

`packages/runtime/src/edit-replace.ts` carried opencode's fuzzy string-edit matchers (line-trimmed, whitespace-normalized, escape-normalized, the last two with cline and gemini-cli lineage) until 2026-09-24, when Edit moved to exact matching with only quote and line-ending normalization. That code was removed, together with its `LICENSE` entries and header-gate exclusion; the file is now Maka source under the ASF header.
