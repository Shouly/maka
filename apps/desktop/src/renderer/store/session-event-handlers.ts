...
export function createAppShellSessionEventHandlers(options: {
    ...
    if (event.type === 'tool_output_delta') {
      ...
      for (let index = events.length - 1; index >= 0; index -= 1) {
        ...
        if (
          ...
          if (chunks > TOOL_STREAM_MAX_CHUNKS || chars > TOOL_STREAM_MAX_TOTAL_CHARS)
  ...
  function terminalRefreshOptions(
    projection: LiveTurnProjection | undefined,
  ): RefreshMessagesOptions | undefined {
  ...
  function handleEvent(sessionId: string, event: SessionEvent) {
    ...
    switch (event.type) {
      ...
      case 'steering_message':
        ...
        break;
      case 'text_complete':
        void refreshMessages(sessionId, { requiredAssistantMessageId: event.messageId }).catch(
          () => false,
        );
        break;
...
function sessionEventDiagnosticDetails(
  ...
  return [
    ...
    ...(event.details === undefined ? [] : [`Details: ${JSON.stringify(event.details)}`]),
  ].join('\n');
}
