...
const ZH_CN = {
  ...
  directPathUnavailable:
    '尚未建立直连。请确认分享者在线，并检查双方的 VPN、代理或防火墙；若网络无法打洞，可使用已知 Mesh 节点的转发。',
  insecureTitle: '此连接未加密',
  insecureBody:
    '访客凭据、完整任务内容和轮次请求可能被同一网络中的第三方截获。仅在你了解并接受风险时继续。',
  ...
  sharedTask: '共享任务',
  newTurnRequestTitle: (count: number) =>
    count === 1 ? '新的轮次请求' : `${count} 个新的轮次请求`,
...
type SessionCollaborationCopy = {
  readonly [Key in keyof typeof ZH_CN]: (typeof ZH_CN)[Key] extends (...args: infer Args) => string
    ? (...args: Args) => string
    : string;
};
...
const ZH_TW = {
  ...
  directPathUnavailable:
    '尚未建立直連。請確認分享者在線，並檢查雙方的 VPN、代理或防火牆；若網路無法打洞，可使用已知 Mesh 節點的轉送。',
  insecureTitle: '此連線未加密',
  insecureBody:
    '訪客憑證、完整任務內容和輪次請求可能被同一網路中的第三方攔截。僅在你了解並接受風險時繼續。',
...
const EN = {
  ...
  invitationHelp:
    'The code contains the connection address and Guest credential, never the Owner credential.',
  ...
  recoveryStartedBody:
    'The task is saved on this device and will appear in the task list when the connection recovers.',
  invalidCode: 'The invitation code is invalid',
  connectionFailed: 'Could not join the shared task',
  directPathUnavailable:
    'No direct connection yet. Check that the owner is online and review VPN, proxy, or firewall settings on both devices. If hole punching is unavailable, use an approved Mesh member for transit.',
  insecureTitle: 'This connection is not encrypted',
  insecureBody:
    'The Guest credential, complete task content, and Turn requests may be intercepted by others on the network. Continue only if you understand and accept the risk.',
  ...
  incompatibleHost:
    'The Maka versions are incompatible. Update Maka and restart Runtime Host on both devices, then retry.',
  alias: 'Alias',
  ...
  turnRequestReconnecting:
    'Reconnecting to the Runtime Host. Your draft is kept and can be sent when the connection returns.',
  ...
  newTurnRequestTitle: (count: number) =>
    count === 1 ? 'New Turn request' : `${count} new Turn requests`,
  ...
  moreTurnRequests: (count: number) =>
    `${count} more ${count === 1 ? 'request' : 'requests'} waiting`,
  pendingTurnRequestCount: (count: number) =>
    `${count} pending Turn ${count === 1 ? 'request' : 'requests'}`,
} satisfies SessionCollaborationCopy;
...
const COPY = {
  ...
  'zh-TW': ZH_TW,
  en: EN,
} satisfies UiCatalog<SessionCollaborationCopy>;
