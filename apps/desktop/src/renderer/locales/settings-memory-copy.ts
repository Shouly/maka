...
function makeCopy(
  ...
  return {
    ...
    countActive: (count, draft) =>
      isZh
        ? `${draft ? '草稿 ' : ''}${count} ${zh('条生效', '則生效')}`
        : `${draft ? 'Draft · ' : ''}${plural(count, 'active entry', 'active entries')}`,
    countArchived: (count, draft) =>
      isZh
        ? `${draft ? '草稿 ' : ''}${count} ${zh('条已归档', '則已歸檔')}`
        : `${draft ? 'Draft · ' : ''}${plural(count, 'archived entry', 'archived entries')}`,
    saveSummary: (active, archived) =>
      isZh
        ? `${zh('当前', '目前')} ${active} ${zh('条生效', '則生效')}${archived > 0 ? ` / ${archived} ${zh('条已归档', '則已歸檔')}` : ''}；${zh('已保留上一版备份。', '已保留上一版備份。')}`
        : `${plural(active, 'active entry', 'active entries')}${archived > 0 ? ` / ${plural(archived, 'archived entry', 'archived entries')}` : ''}; the previous version was backed up.`,
    backupSummary: (active, archived) =>
      isZh
        ? `${active} ${zh('条生效', '則生效')}${archived > 0 ? ` / ${archived} ${zh('条已归档', '則已歸檔')}` : ''}`
        : `${plural(active, 'active entry', 'active entries')}${archived > 0 ? ` / ${plural(archived, 'archived entry', 'archived entries')}` : ''}`,
    countEntries: (count) =>
      isZh ? `${count} ${zh('条记忆', '則記憶')}` : plural(count, 'memory', 'memories'),
    countMatches: (filtered, total) =>
      isZh ? `${filtered} / ${total} ${zh('条匹配', '則符合')}` : `${filtered} / ${total} matching`,
    listAria: (title) => (isZh ? `${title}${zh('列表', '清單')}` : `${title} list`),
    entryActionsAria: (title) =>
      isZh ? `${title} ${zh('记忆操作', '記憶操作')}` : `${title} memory actions`,
    entryActionAria: (action, identity) =>
      isZh ? `${action}：${identity}` : `${action}: ${identity}`,
    openBackupAria: (label) =>
      isZh ? `${zh('打开备份候选', '開啟備份候選')} ${label}` : `Open backup candidate ${label}`,
    restoreBackupAria: (label) =>
      isZh ? `${zh('恢复备份候选', '還原備份候選')} ${label}` : `Restore backup candidate ${label}`,
    copyBackupAria: (label) =>
      isZh
        ? `${zh('复制备份候选引用', '複製備份候選引用')} ${label}`
        : `Copy backup candidate reference ${label}`,
    draftStatusAria: (action) =>
      isZh
        ? `${action}，${zh('保存前不会写入', '儲存前不會寫入')} MEMORY.md`
        : `${action}; MEMORY.md is not written until you save`,
    restoreLatestDescription: (label) =>
      isZh
        ? `${zh('会先备份当前 MEMORY.md，再用最近一次备份覆盖当前文件。将恢复', '會先備份目前的 MEMORY.md，再以最近一次備份覆蓋目前檔案。將還原')}：${label}`
    ...
    restoreCandidateDescription: (label) =>
      isZh
        ? `${zh('会先备份当前 MEMORY.md，再用选中的备份覆盖当前文件。将恢复', '會先備份目前的 MEMORY.md，再以選取的備份覆蓋目前檔案。將還原')}：${label}`
        : `The current MEMORY.md will be backed up before the selected backup replaces it. Restore: ${label}`,
    redactedDetail: (summary) =>
      isZh
        ? `${zh('写入前已替换疑似', '寫入前已遮蔽疑似')} token、API key ${zh('或密码', '或密碼')}；${summary}`
        : `Suspected tokens, API keys, or passwords were redacted before writing; ${summary}`,
    openBackupFailed: (kind) =>
      isZh ? `${zh('打开', '開啟')}${kind}${zh('失败', '失敗')}` : `Failed to open ${kind}`,
    previewTruncated: (limit) =>
      isZh
        ? `${zh('预览已按', '預覽已依')} ${limit} ${zh('字符上限截断', '字元上限截斷')}`
        : `Preview truncated at the ${limit}-character limit`,
    previewUsage: (length, limit) =>
      isZh
        ? `${zh('预览', '預覽')} ${length} / ${limit} ${zh('字符', '字元')}`
        : `Preview ${length} / ${limit} characters`,
    previewLimit: (limit) =>
      isZh
        ? `prompt ${zh('上限', '上限')} ${limit} ${zh('字符', '字元')}`
        : `Prompt limit: ${limit} characters`,
  };
}
