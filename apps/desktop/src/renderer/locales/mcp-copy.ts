...
const MCP_COPY = {
  'zh-CN': {
    errors: {
      ...
      test: 'MCP 测试失败',
      remove: '删除 MCP 失败',
      unavailableStatus: 'Server 没有返回可用状态。',
      mapLine: (line) => `第 ${line} 行应为 KEY=value`,
      importJson: 'MCP 配置必须是有效的 JSON',
      importObject: 'MCP JSON 必须是 object',
      importVersion: (version) => `不支持 MCP 配置版本 ${version}，当前支持 version 1、2 和 3`,
    ...
    toast: {
      ...
      imported: '已导入 MCP',
      importedDetail: (count) => `本次导入 ${count} 个 server。`,
      connectionOk: 'MCP 连接正常',
    ...
    page: {
      ...
      workspaceAria: 'MCP 市场与已安装项',
      toolbarAria: 'MCP 浏览操作',
      setupTitle: '把 Maka 连接到你的工作环境',
      setupDescription: '从精选模板开始，或添加任意 stdio、Streamable HTTP 与 SSE server。',
      localStdio: '本地 stdio',
      categoriesAria: 'MCP 分类',
      market: '市场',
      installed: '已安装',
      searchPlaceholder: '搜索 MCP…',
      searchAria: '搜索 MCP',
      noMarket: '没有找到匹配的 MCP',
      noMarketDetail: (query) => `换一个关键词，或清空「${query}」查看全部模板。`,
      clearSearch: '清空搜索',
      loading: '正在读取 MCP 配置…',
      noInstalled: '还没有安装 MCP',
      noInstalledDetail: '从市场选择模板，或手动添加你自己的 server。',
      browseMarket: '浏览市场',
      noInstalledMatch: '没有匹配的已安装 MCP',
    ...
    editor: {
      ...
      editTitle: (id) => `编辑 ${id}`,
      addTitle: '添加 MCP',
      importSubtitle: '粘贴 mcpServers 配置，同名 server 会被更新。',
      manualSubtitle: '配置保存在当前工作区的 mcp.json。',
      modeAria: 'MCP 添加方式',
      manual: '手动配置',
      pasteJson: '粘贴 JSON',
      jsonConfig: 'JSON 配置',
      jsonHelp:
        '支持完整 mcpServers 配置或直接的 server map。未在本次导入中出现的已有 MCP 会保留。',
      cancel: '取消',
      importConnect: '导入并连接',
      transportAria: '连接方式',
      localStdio: '本地 stdio',
      ...
      environment: '环境变量',
      environmentHelp: '每行一个 KEY=value；按 MCP 要求填写。',
      url: 'MCP URL',
      headers: 'HTTP 请求头',
  ...
  'zh-TW': {
    errors: {
      ...
      test: 'MCP 測試失敗',
      remove: '刪除 MCP 失敗',
      unavailableStatus: 'Server 沒有返回可用狀態。',
      mapLine: (line) => `第 ${line} 行應為 KEY=value`,
      importJson: 'MCP 設定必須是有效的 JSON',
      importObject: 'MCP JSON 必須是 object',
      importVersion: (version) => `不支援 MCP 設定版本 ${version}，目前支援 version 1、2 和 3`,
    ...
    toast: {
      ...
      imported: '已匯入 MCP',
      importedDetail: (count) => `本次匯入 ${count} 個 server。`,
      connectionOk: 'MCP 連線正常',
      toolLatency: (count, latencyMs) => `${count} 個工具 · ${latencyMs} ms`,
      connectionFailed: 'MCP 連線失敗',
      removed: 'MCP 已刪除',
    },
    remove: {
      title: (id) => `刪除 MCP「${id}」？`,
    ...
    page: {
      ...
      workspaceAria: 'MCP 市場與已安裝項',
      toolbarAria: 'MCP 瀏覽操作',
      setupTitle: '把 Maka 連線到你的工作環境',
      setupDescription: '從精選模板開始，或新增任意 stdio、Streamable HTTP 與 SSE server。',
      localStdio: '本地 stdio',
      categoriesAria: 'MCP 分類',
      market: '市場',
      installed: '已安裝',
      searchPlaceholder: '搜尋 MCP…',
      searchAria: '搜尋 MCP',
      noMarket: '沒有找到符合的 MCP',
      noMarketDetail: (query) => `換一個關鍵詞，或清空「${query}」檢視全部模板。`,
      clearSearch: '清空搜尋',
      loading: '正在讀取 MCP 設定…',
      noInstalled: '還沒有安裝 MCP',
      noInstalledDetail: '從市場選擇模板，或手動新增你自己的 server。',
      browseMarket: '瀏覽市場',
      noInstalledMatch: '沒有符合的已安裝 MCP',
    ...
    editor: {
      ...
      editTitle: (id) => `編輯 ${id}`,
      addTitle: '新增 MCP',
      importSubtitle: '貼上 mcpServers 設定，同名 server 會被更新。',
      manualSubtitle: '設定儲存在目前工作區的 mcp.json。',
      modeAria: 'MCP 新增方式',
      manual: '手動設定',
      pasteJson: '貼上 JSON',
      jsonConfig: 'JSON 設定',
      jsonHelp:
        '支援完整 mcpServers 設定或直接的 server map。未在本次匯入中出現的已有 MCP 會保留。',
      cancel: '取消',
      importConnect: '匯入並連線',
      transportAria: '連線方式',
      localStdio: '本地 stdio',
      ...
      environment: '環境變數',
      environmentHelp: '每行一個 KEY=value；按 MCP 要求填寫。',
      url: 'MCP URL',
      headers: 'HTTP 請求頭',
  ...
  en: {
    errors: {
      load: 'Failed to load MCP',
      install: (name) => `Failed to install ${name}`,
      cancelInstall: (name) => `Failed to cancel installation of ${name}`,
      save: 'Failed to save MCP',
      import: 'Failed to import MCP',
      update: 'Failed to update MCP',
      test: 'MCP test failed',
      remove: 'Failed to delete MCP',
      unavailableStatus: 'The server did not return an available status.',
      mapLine: (line) => `Line ${line} must use KEY=value`,
      importJson: 'MCP configuration must be valid JSON',
      importObject: 'MCP JSON must be an object',
      importVersion: (version) =>
        `Unsupported MCP config version ${version}; versions 1, 2, and 3 are supported`,
    ...
    toast: {
      ...
      imported: 'MCP imported',
      importedDetail: (count) => `Imported ${count} ${count === 1 ? 'server' : 'servers'}.`,
      connectionOk: 'MCP connection healthy',
    ...
    page: {
      ...
      workspaceAria: 'MCP marketplace and installed servers',
      toolbarAria: 'MCP browser controls',
      ...
      categoriesAria: 'MCP categories',
      market: 'Marketplace',
      installed: 'Installed',
      searchPlaceholder: 'Search MCP…',
      searchAria: 'Search MCP',
      noMarket: 'No matching MCP servers',
      ...
      noInstalledDetail: 'Choose a template from the marketplace, or add your own server manually.',
      browseMarket: 'Browse marketplace',
      noInstalledMatch: 'No matching installed MCP servers',
      noInstalledMatchDetail: (query) =>
        `Try another keyword, or clear “${query}” to view every installed server.`,
    },
    detail: {
    ...
    editor: {
      ...
      addTitle: 'Add MCP',
      importSubtitle:
        'Paste an mcpServers configuration; servers with matching names will be updated.',
      manualSubtitle: 'Configuration is saved in mcp.json for the current workspace.',
      modeAria: 'MCP add method',
      manual: 'Manual configuration',
      pasteJson: 'Paste JSON',
      jsonConfig: 'JSON configuration',
      jsonHelp:
        'Supports a complete mcpServers configuration or a server map. Existing MCP servers omitted from this import are preserved.',
      cancel: 'Cancel',
      ...
      environmentHelp: 'One KEY=value entry per line; complete the variables required by this MCP.',
      url: 'MCP URL',
