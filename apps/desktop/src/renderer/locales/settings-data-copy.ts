...
const SETTINGS_DATA_COPY = {
  'zh-CN': {
    ...
    importSummary: {
      connections: (created, overwritten, skipped) =>
    ...
    rows: {
      ...
      history: '输入历史',
      historyDetail:
        '上箭头 / 下箭头调出的已发送提示词记录，保存在本机、重启后仍在。清空后无法恢复。',
    },
    actionsAria: '工作区数据操作',
    opening: '打开中…',
  ...
  'zh-TW': {
    ...
    loadFailed: '載入資料目錄失敗',
    openFailed: (label) => `無法開啟${label}`,
    pathCopied: '已複製工作區路徑',
    copyFailed: '複製失敗',
    copyFailedDetail: '剪貼簿不可用或被系統拒絕。',
    historyCleared: '已清空輸入歷史',
    historyClearedDetail: '已傳送的提示詞記錄已從本機移除。',
    selectCategory: '請至少選擇一個類別',
    exported: '已匯出設定',
    exportedDetail: (items) => `包含：${items.join('、')}`,
    exportFailed: '匯出失敗',
    noCategories: '未選擇任何類別',
    tryAgain: '請稍後重試',
    imported: '已匯入設定',
    importFailed: '匯入失敗',
    importFailures: {
    ...
    rows: {
      ...
      loadValueFailed: '載入失敗',
      loading: '正在載入…',
      history: '輸入歷史',
      historyDetail:
        '上箭頭 / 下箭頭調出的已傳送提示詞記錄，儲存在本機、重啟後仍在。清空後無法恢復。',
    },
    actionsAria: '工作區資料操作',
    opening: '開啟中…',
    openWorkspace: '開啟工作區資料夾',
    copying: '複製中…',
    copyPath: '複製路徑',
    clearing: '清空中…',
    clearHistory: '清空輸入歷史',
    ...
    pathLoadFailed: (error) => `無法載入工作區路徑：${error}`,
    configAria: '設定匯入匯出',
    configTitle: '設定匯入匯出',
    configHelp: '勾選要匯出的內容，生成一個 JSON 備份檔案；換機或重灌時可再匯入。預設不含金鑰。',
    categoryAria: '選擇匯出內容',
    sensitiveWarning:
      '⚠️ 金鑰將以明文寫入匯出檔案。任何拿到該檔案的人都能使用這些金鑰，請妥善保管、不要分享。',
  ...
  en: {
    ...
    rows: {
      ...
      historyDetail:
        'Previously sent prompts recalled with the Up and Down arrows are kept on this machine and persist across restarts. Clearing them cannot be undone.',
    },
    ...
    sensitiveWarning:
      '⚠️ Secrets will be written to the export file as plain text. Anyone with this file can use them. Store it securely and do not share it.',
