...
import { listTurns } from '../../bridge/sessions.js';
...
export async function readSettledMessages(
  ...
  // Through the bridge: only `bridge/` reaches the preload from the renderer.
  return readSettledMessagesFrom(
    { transcripts: { open: openTranscript }, sessions: { listTurns } },
    sessionId,
    options,
  );
}
...
function transcriptRecordsTerminalTurn(
  ...
  return messages.some(
    (message) =>
      message.type === 'turn_state' && message.turnId === turnId && message.status !== 'running',
  );
}
