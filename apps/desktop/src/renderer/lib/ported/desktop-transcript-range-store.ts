...
import { TranscriptReadSupersededError } from './transcript-reading-position.js';
...
export function createDesktopTranscriptRangeController(
  ...
  const command = async (
    ...
    const navigation = replace
      ? store.navigate()
      : (store.pendingNavigation() ?? store.navigation());
    const opening = handle;
    const isCurrent = () =>
      !closed && opening === handle && (!replace || store.pendingNavigation() === navigation);
    try {
...
export function createDesktopTranscriptReconnectRecovery(options: {
  ...
  const recover = () => {
    if (
      ...
      !needsRecovery ||
      recoveryTask ||
      attemptedReadinessGeneration === readinessGeneration
    )
      return;
...
export function createRecoveringDesktopTranscriptRangeController(
   ...
   */
  const superseding = <T>(run: () => Promise<T>): Promise<T> =>
  ...
  return {
    ...
    loadAround: (sequence, maxBytes) =>
      superseding(() => controller.loadAround(sequence, maxBytes)),
  ...
  accept(batch: DesktopTranscriptBatchPayload): boolean {
    ...
    if (!assembly) {
      ...
      assembly = {
        ...
        collects:
          kind !== 'tail' || this.#tail.overlay.size > 0 || this.#joinsTail(batch.coversFrom),
    ...
    for (const fragment of batch.fragments)
      this.#acceptFragment(assembly, fragment, assembly.collects);
...
function continuesAssembly(
  ...
  return (
    assembly.sessionId === batch.sessionId &&
