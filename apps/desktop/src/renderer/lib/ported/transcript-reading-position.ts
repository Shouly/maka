...
/**
 * Where the reader was parked in a transcript. Inlined here in the enterprise
 * renderer rewrite (Phase 0a) because the old `session-ui-state.ts` store was
 * deleted with the rest of the legacy shell; Phase 1 re-homes it in `store/`.
 */
...
export function createTranscriptRestoreLifecycle() {
  ...
  return {
    request(input: {
      ...
      const search =
        input.searchTarget?.sessionId === input.sessionId ? input.searchTarget : undefined;
        ...
        activation = {
          ...
          command:
            input.sessionId && target
              ? {
                  target: { turnId: target.turnId, sequence: target.sequence },
                  fromSearch: Boolean(search),
                  completed: false,
                }
              : undefined,
        };
...
export async function prepareTranscriptForSend<Message>(options: {
  ...
  controller: {
    current: (TranscriptRangeController<Message> & { loadLatest(): Promise<void> }) | undefined;
  };
...
export function restoreSessionTranscriptRange<Message>(options: {
  ...
  const current = (): boolean =>
    options.lifecycle.isCurrent(command) &&
    command.attempt === attempt &&
  ...
  try {
    ...
    admitted =
      residentSequence !== null || sequence === undefined
  ...
  void admitted
    .then(() => {
      ...
      if (
        options.isLiveTurn?.(sessionId, target.turnId) ||
        controller.store
          .snapshot()
          .messages.some(
            (message) =>
