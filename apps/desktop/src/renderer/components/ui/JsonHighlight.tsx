...
const JsonHighlight = memo(function JsonHighlight({
  content,
  title,
  className = '',
  compact = false
  ...
  return (
    <div className={`relative bg-surface-1 rounded-lg overflow-hidden custom-json-highlight ${className}`}>
      {/* 复制按钮 - 固定在右上角 */}
      <button
        type="button"
        onClick={handleCopy}
      ...
      {title && (
        <div className={compact ? "px-2 py-0.5" : "px-3 py-1"}>
          <span className={`${compact ? 'text-[10px]' : 'text-[11px]'}  font-medium text-text-muted`}>{title}</span>
        </div>
      )}
      ...
      {highlightedContent.isJson ? (
        ...
        <pre className={`${compact ? 'text-xs px-2 py-0.5' : 'text-sm px-3 py-1'} text-text-secondary font-mono font-normal whitespace-pre-wrap break-words`}>
          {content}
        </pre>
      )}
    </div>
