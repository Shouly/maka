...
/**
 * The plain text a row's copy action puts on the clipboard: what was asked
 * for, then what came back. Never the raw JSON of the result envelope — the
 * quiet formatter is what the panel shows, so it is what a copy should carry.
 */
export function toolCopyText(item: ToolActivityItem, locale: UiLocale): string {
  const parts: string[] = [toolRowTitle(item, locale)];
  const result = item.result;
  if (result) {
    switch (result.kind) {
      case 'text':
      case 'summary':
        parts.push(result.kind === 'text' ? result.text : result.summarized);
        break;
      case 'file_diff':
      ...
      case 'terminal':
      case 'shell_run': {
        parts.push(result.cmd);
        const output = result.output;
        if (output && output.mode === 'pipes') {
          if (output.stdout) parts.push(output.stdout);
          if (output.stderr) parts.push(output.stderr);
        } else if (output && output.mode === 'pty') {
          parts.push(output.screen || output.scrollback);
        }
        break;
      }
      case 'json':
      ...
      case 'subagent':
        ...
        break;
      case 'agent_swarm':
        parts.push(result.items.map((row) => row.summary).join('\n\n'));
        break;
      case 'rive_workflow':
        parts.push(result.summary);
        if (result.stdoutTail) parts.push(result.stdoutTail);
  ...
  for (const chunk of item.outputChunks ?? []) parts.push(chunk.text);
