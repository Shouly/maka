...
import { test, expect, COMPOSER_INPUT, ensureSidebarExpanded } from './fixtures';
...
test('proxy password drafts save once, reload safely, and authenticate offline', async ({
  ...
  try {
    ...
    const passwordInput = page.getByLabel('凭据值', { exact: true });
    await passwordInput.fill(password);
    await expect(passwordInput).toHaveAttribute('type', 'password');
    await expect
      .poll(() =>
        page.evaluate(
          async () => (await window.maka.settings.get()).network.proxy.passwordConfigured,
        ),
      )
      .toBe(false);
    ...
    const reloadedPassword = page.getByPlaceholder('密码已保存；输入新密码以替换');
    await expect(reloadedPassword).toHaveValue('');
    await reloadedPassword.fill(replacementPassword);
    await reloadedPassword.press('Tab');
    await expect(reloadedPassword).toHaveValue('');
